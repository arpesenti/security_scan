...
class TestCacheKey(unittest.TestCase):
    ...
    def test_legacy_scan_key_equals_old_format(self):
        # The legacy key is the hash format used before --thinking was
        # added to the cache key. It must produce a *different* hash
        # than the new key with thinking="" (which has a trailing
        # colon), otherwise the fallback would always be a no-op.
        new = ss.file_key("src/foo.py", "injection", "chash", "phash", "")
        legacy = ss._legacy_scan_key("src/foo.py", "injection", "chash", "phash")
        self.assertNotEqual(new, legacy)
        # And the legacy key must NOT have a trailing colon in its
        # inputs — that was the whole point of the format change.
        self.assertEqual(
            legacy,
            __import__("hashlib").md5(
                b"injection:src/foo.py:chash:phash"
            ).hexdigest(),
        )

    def test_legacy_verify_key_equals_old_format(self):
        new = ss.verify_file_key("a.py", "scanner", "chash", "fsig", "vph", "")
        legacy = ss._legacy_verify_key("a.py", "scanner", "chash", "fsig", "vph")
        self.assertNotEqual(new, legacy)
        self.assertEqual(
            legacy,
            __import__("hashlib").md5(
                b"verify:scanner:a.py:chash:fsig:vph"
            ).hexdigest(),
        )
...
class TestFindCacheFile(unittest.TestCase):
    """_find_cache_file is the read-side helper that powers the legacy
    fallback. It must: prefer the new key, fall back to the legacy
    key when the new one is missing, return None when neither is
    usable, and not error on either key alone."""
    ...
    def _write(self, key, status="ok"):
        (self.cache / f"{key}.json").write_text(json.dumps({
            "status": status, "verifications": {},
        }))

    def test_returns_none_when_dir_empty(self):
        self.assertIsNone(ss._find_cache_file(self.cache, "primary"))

    def test_returns_primary_when_present(self):
        self._write("primary", status="ok")
        result = ss._find_cache_file(self.cache, "primary", "legacy")
        self.assertEqual(result, self.cache / "primary.json")

    def test_falls_back_to_legacy(self):
        # Only the legacy file exists — the fallback must surface it
        # so pre-thinking caches are still readable.
        self._write("legacy", status="ok")
        result = ss._find_cache_file(self.cache, "primary", "legacy")
        self.assertEqual(result, self.cache / "legacy.json")

    def test_primary_wins_over_legacy(self):
        # If both exist (the post-migration state), the new key wins
        # and the legacy file is left to be garbage-collected later.
        self._write("primary", status="ok")
        self._write("legacy", status="ok")
        result = ss._find_cache_file(self.cache, "primary", "legacy")
        self.assertEqual(result, self.cache / "primary.json")

    def test_failed_primary_does_not_block_legacy_fallback(self):
        # A primary file with status=error is a miss; the fallback
        # should still fire and return the legacy success.
        self._write("primary", status="error")
        self._write("legacy", status="ok")
        result = ss._find_cache_file(self.cache, "primary", "legacy")
        self.assertEqual(result, self.cache / "legacy.json")
    ...
    def test_returns_none_when_both_unusable(self):
        self._write("primary", status="error")
        self._write("legacy", status="error")
        self.assertIsNone(ss._find_cache_file(self.cache, "primary", "legacy"))

    def test_no_legacy_arg_means_no_fallback(self):
        # When the caller doesn't pass a legacy key (e.g. callers
        # that don't have one to offer), the helper must not invent
        # one and silently fall through.
        self._write("legacy", status="ok")
        self.assertIsNone(ss._find_cache_file(self.cache, "primary"))

    def test_corrupt_legacy_falls_through(self):
        # A corrupt legacy file must not crash the lookup; it should
        # be treated as a miss and the helper should return None.
        (self.cache / "legacy.json").write_text("not json {{{")
        self.assertIsNone(ss._find_cache_file(self.cache, "primary", "legacy"))
...
class TestPhaseToolsCacheRouting(unittest.TestCase):
    ...
    def test_legacy_scan_cache_is_used_by_run_scanner(self):
        # Pre-thinking cache: a scan result file keyed without the
        # `:thinking` field in the hash. The pre-check must treat it
        # as a hit, otherwise the user re-scans every file.
        c_hash = ss.content_hash((self.root / "code.py").read_bytes())
        legacy_key = ss._legacy_scan_key("code.py", "injection", c_hash, self.p_hash)
        results_dir = self.state / "injection" / "results"
        results_dir.mkdir(parents=True, exist_ok=True)
        (results_dir / f"{legacy_key}.json").write_text(json.dumps({
            "file": "code.py", "scanner": "injection", "status": "ok",
            "result": [{"line": 1, "code": "x"}],
            "content_hash": c_hash, "prompt_hash": self.p_hash,
        }))
        all_files, ext_map, name_map = ss.find_all_files(self.root)
        with patch.object(ss, "call_pi") as mock:
            ss.run_scanner(
                "B3", self.cfg, ext_map, name_map, self.discovery,
                self.root, self.state, self.sessions,
                concurrency=1, max_files=0, rescan=False, dry_run=False,
                tools=None, thinking="off",
            )
        # Legacy file is a hit — no pi call, file is reported as cached.
        self.assertEqual(mock.call_count, 0)

    def test_legacy_scan_cache_hit_writes_new_key_on_run(self):
        # After the legacy fallback fires, the new write must still
        # target the new key — we never want to overwrite a legacy
        # file (it would shadow the migration). This test exercises
        # the full scan_file path with a real pi call.
        c_hash = ss.content_hash((self.root / "code.py").read_bytes())
        legacy_key = ss._legacy_scan_key("code.py", "injection", c_hash, self.p_hash)
        results_dir = self.state / "injection" / "results"
        results_dir.mkdir(parents=True, exist_ok=True)
        ...
        # --rescan forced a re-scan; the new result must be at the
        # new key, not the legacy one.
        new_key = ss.file_key("code.py", "injection", c_hash, self.p_hash, "off")
        self.assertTrue((results_dir / f"{new_key}.json").exists())
        self.assertTrue((results_dir / f"{legacy_key}.json").exists())

    def test_legacy_verify_cache_is_used_by_run_verification(self):
        # Mirror of the scan test: a pre-thinking verify cache must
        # be treated as a hit by run_verification's pre-check. The
        # verify cache key uses the verify prompt hash, not the
        # scanner prompt hash — get the right one.
        c_hash = ss.content_hash((self.root / "code.py").read_bytes())
        findings = [{"line": 1, "severity": "High", "code": "x",
                     "explanation": "", "fix": ""}]
        f_sig = ss.findings_signature(findings)
        v_prompt_hash = ss.prompt_hash(ss.load_verify_prompt())
        legacy_key = ss._legacy_verify_key(
            "code.py", "injection", c_hash, f_sig, v_prompt_hash,
        )
        verify_dir = self.state / "injection" / "verifications"
        verify_dir.mkdir(parents=True, exist_ok=True)
        (verify_dir / f"{legacy_key}.json").write_text(json.dumps({
            "file": "code.py", "scanner": "injection", "status": "ok",
            "verifications": {"1": {"confidence": "High"}},
        }))
        self._seed_scan_cache("results", findings)
        with patch.object(ss, "call_pi") as mock:
            ss.run_verification(
                "B3", self.cfg, self.root, self.state, self.sessions,
                concurrency=1, reverify=False, dry_run=False,
                tools=None, thinking="medium",
            )
        # No pi call: the legacy fallback matched the cache.
        self.assertEqual(mock.call_count, 0)

    def test_legacy_verify_cache_is_loaded_by_report(self):
        # build_report reads verify verdicts via load_verification_for_file.
        # A pre-thinking cache file must be picked up so the report
        # actually shows confidence/exploitability columns populated.
        c_hash = ss.content_hash((self.root / "code.py").read_bytes())
        findings = [{"line": 1, "severity": "High", "code": "x",
                     "explanation": "", "fix": ""}]
        f_sig = ss.findings_signature(findings)
        ...
        result = ss.load_verification_for_file(
            "injection", "code.py", c_hash, findings, verify_dir, v_prompt_hash,
        )
        # The legacy fallback returns the verdicts, not None.
        self.assertIsNotNone(result)
        self.assertEqual(result["1"]["confidence"], "High")
